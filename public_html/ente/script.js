const DotField = {
    ...
    resize() {
        ...
        for (let y = -1; y < rows; y++) {
            for (let x = -1; x < cols; x++) this.dots.push({ x: x * spacing, y: y * spacing });
        }
    },
    ...
    updateShields() {
        ...
        this.shields = Array.from(shielded).map(el => {
            ...
            return { left: rect.left - 22, right: rect.right + 22, top: rect.top - 12, bottom: rect.bottom + 12 };
        });
    },
    ...
    shieldAmount(dot) {
        ...
        this.shields.forEach(rect => {
            ...
            const fade = Math.max(0, 1 - Math.sqrt(dx * dx + dy * dy) / 30);
    ...
    draw() {
        ...
        this.dots.forEach(dot => {
            ...
            const pull = this.pointer.active && distSq < radiusSq ? 1 - Math.sqrt(distSq) / radius : 0;
            const eased = pull * pull * (3 - 2 * pull);
            const visibility = 1 - this.shieldAmount(dot) * 0.82;
            const size = (1 + eased * 2.6) * visibility;
            const whiteAlpha = (0.16 + eased * 0.12) * visibility;
            const glowAlpha = eased * 0.92 * visibility;
            if (whiteAlpha < 0.012 && glowAlpha < 0.012) return;

            this.ctx.beginPath();
            this.ctx.arc(dot.x, dot.y, size, 0, Math.PI * 2);
            this.ctx.fillStyle = `rgba(255, 255, 255, ${Math.max(whiteAlpha, glowAlpha)})`;
            this.ctx.fill();
        });
