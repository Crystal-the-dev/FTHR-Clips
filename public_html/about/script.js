...
const LanguageSwitch = {
    ...
    set(lang) {
        ...
        document.title = lang === 'de' ? 'Impressum | FTHR-Software' : 'Legal Notice | FTHR-Software';
        if (title) title.textContent = lang === 'de' ? 'Impressum' : 'Legal Notice';
        if (tradeLabel) tradeLabel.textContent = lang === 'de' ? 'Gewerbeform' : 'Business Type';
        if (tradeStatus) {
            tradeStatus.textContent = lang === 'de'
                ? 'Nicht eingetragenes Einzelgewerbe'
    ...
    init() {
        this.buttons = Array.from(document.querySelectorAll('[data-lang-button]'));
        this.panels = Array.from(document.querySelectorAll('[data-lang-panel]'));
        this.buttons.forEach((button) => {
            button.addEventListener('click', () => this.set(button.dataset.langButton));
        });
        this.set('de');
    }
};
