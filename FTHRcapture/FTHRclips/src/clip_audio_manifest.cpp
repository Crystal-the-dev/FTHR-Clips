...
bool WriteClipAudioManifest(
    ...
    std::string* error) {
    if (tracks.empty() || tracks.size() > kMaxRetainedAudioSources || !IsUuid(transaction_id)) {
        if (error) *error = "audio manifest needs one to eight tracks and a valid transaction UUID";
        return false;
    }
