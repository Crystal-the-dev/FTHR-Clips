...
namespace fthr {
    ...
    EncodedRingSnapshot EncodedRingBuffer::TakeSnapshot(size_t frame_count) const {
        ...
        //
        // Guard: if the engine just started and hasn't buffered fps_ frames yet,
        // current_head < safety and safe_end would underflow (uint64 wrap).
        // Return empty — the MuxEncodedClip caller handles this gracefully.
        const size_t   safety     = static_cast<size_t>(fps_);
        const uint64_t safety_u64 = static_cast<uint64_t>(safety);

        if (current_head <= safety_u64) {
            std::cerr << "[EncodedRingBuffer] TakeSnapshot: engine just started "
                      << "(" << current_head << " frames buffered, "
                      << safety << " minimum needed) — returning empty snapshot"
                      << std::endl;
            return snap;
        }

        const uint64_t safe_end   = current_head - safety_u64;
        ...
        const uint64_t start_pos = safe_end - static_cast<uint64_t>(frames_to_copy);
